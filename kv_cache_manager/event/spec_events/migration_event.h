...
public:
    ...
    void SetAdditionalArgs(std::int64_t block_key, const std::string &dst_storage, std::int64_t latency_since_mark_ms) {
        block_key_ = block_key;
        dst_storage_ = dst_storage;
        latency_since_mark_ms_ = latency_since_mark_ms;
    }
    ...
    void ToRapidWriter(rapidjson::Writer<rapidjson::StringBuffer> &writer) const noexcept override {
        ...
        Put(writer, "block_key", block_key_);
        Put(writer, "dst_storage", dst_storage_);
        Put(writer, "latency_since_mark_ms", latency_since_mark_ms_);
    }

private:
    std::int64_t block_key_{0};
    std::string dst_storage_;
    std::int64_t latency_since_mark_ms_{0};
};
...
public:
    ...
    void SetAdditionalArgs(std::int64_t block_key, const std::string &dst_storage, std::int64_t mark_age_ms) {
        block_key_ = block_key;
        dst_storage_ = dst_storage;
        mark_age_ms_ = mark_age_ms;
    }
    ...
    void ToRapidWriter(rapidjson::Writer<rapidjson::StringBuffer> &writer) const noexcept override {
        ...
        Put(writer, "block_key", block_key_);
        Put(writer, "dst_storage", dst_storage_);
        Put(writer, "mark_age_ms", mark_age_ms_);
    }

private:
    std::int64_t block_key_{0};
    std::string dst_storage_;
    std::int64_t mark_age_ms_{0};
};
