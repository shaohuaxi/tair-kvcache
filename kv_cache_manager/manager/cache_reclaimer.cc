...
bool CacheReclaimer::FilterLocID(RequestContext *request_context,
    ...
    for (const auto &loc_map : loc_maps) {
        ...
        // 总水位超限分支：若 block 同时有冷层与非冷层(热/源)副本，则只淘汰热副本、保留冷副本。
        ...
        for (const auto &[_, loc_ptr] : loc_map) {
            ...
            if (loc.status() == CacheLocationStatus::CLS_SERVING || is_orphaned_writing) {
                ...
                } else if (keep_cold_evict_hot && loc_on_cold_storage(loc)) {
...
void CacheReclaimer::TryMigrateOnGroup(const std::shared_ptr<RequestContext> &request_context,
    ...
    for (const auto &strategy : strategies) {
        ...
        const DataStorageType src_type = backend->GetType();
        ...
        const auto configured_copy_concurrency = strategy->methods().copy().max_concurrency();
        const std::size_t max_concurrent_copy =
            strategy->methods().copy().enabled() && configured_copy_concurrency > 0
                ? static_cast<std::size_t>(configured_copy_concurrency)
                : 0;
        ...
        LOG_WITH_GR(DEBUG,
                    ...
                    "max_copy_concurrency [%zu] active_copy [%zu] available_copy_slots [%zu]",
                    src_name.c_str(),
...
std::size_t CacheReclaimer::MigrateByStrategyOnBatch(const std::shared_ptr<RequestContext> &request_context,
    ...
    for (std::size_t i = 0; i != batch.size(); ++i) {
        ...
        const auto admission = migration_manager_->CheckCopyAdmission(block_key, loc_maps[i], src_name, dst_name);
    ...
    if (!mark_keys.empty()) {
        migration_manager_->MarkForTieredWrite(ins_id, mark_keys, dst_name);
...
std::size_t CacheReclaimer::MigrateByStrategy(const std::shared_ptr<RequestContext> &request_context,
                                              const std::shared_ptr<const InstanceInfo> &instance_info,
                                              const MigrationStrategy &strategy,
                                              const std::size_t available_copy_slots) noexcept {
    if (!IsRunning() || IsPaused()) {
        return 0;
    }
    if (instance_info == nullptr || migration_manager_ == nullptr) {
    ...
    std::vector<std::int64_t> batch;
    std::vector<CacheLocationMap> loc_maps;
    if (!BuildMigrationBatch(request_context, instance_info, batch, loc_maps)) {
        return 0;
    }
    return MigrateByStrategyOnBatch(request_context, instance_info, strategy, available_copy_slots, batch, loc_maps);
}
...
bool CacheReclaimer::TryReclaimOnGroup(const std::shared_ptr<RequestContext> &request_context,
    ...
    // 多层存储迁移（水位触发）：与驱逐正交、并行；超过 migration_threshold 后按固定并发上限投递。
    // 在"是否触发驱逐"的早返回之前独立评估，因为迁移可能在尚未触发驱逐时就已需要进行。
    if (migration_manager_ != nullptr && cache_config->migration_strategies().size() > 0) {
