...
private:
    ...
    /**
     * @brief 对单个 instance 执行一条迁移策略：采样(A) → LRU 取最冷 batch → 准入过滤 → 按 methods
     * 分发（copy: MigrationManager::BatchSubmit，受 available_copy_slots 限制；mark: MarkForTieredWrite）。
     * @return 实际提交的 Copy 任务数。
     */
    std::size_t MigrateByStrategy(const std::shared_ptr<RequestContext> &request_context,
                                  const std::shared_ptr<const InstanceInfo> &instance_info,
                                  const MigrationStrategy &strategy,
                                  std::size_t available_copy_slots) noexcept;
